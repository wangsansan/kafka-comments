...
class SocketServer(val config: KafkaConfig, val metrics: Metrics, val time: Time, val credentialProvider: CredentialProvider) extends Logging with KafkaMetricsGroup {
  ...
  def startup(startupProcessors: Boolean = true) {
    this.synchronized {
      ...
      // 创建socketServer的acceptor和processor
  ...
  private def createAcceptorAndProcessors(processorsPerListener: Int,
    ...
    // endpoint就是在config文件里配置的用来进行accept的端口号
...
private[kafka] class Acceptor(val endPoint: EndPoint,
          ...
          if (ready > 0) {
            ...
            while (iter.hasNext && isRunning) {
              try {
                ...
                if (key.isAcceptable) {
                  /**
                   * 轮询当前的acceptor拥有的processor
                   * 将当前可accept的socket交给processor处理
                   */
  ...
  def accept(key: SelectionKey, processor: Processor) {
    ...
    try {
      ...
      /**
       * 把对应的新socket加入到processor的 newConnections 里
       */
    ...
    try {
      while (isRunning) {
        try {
          ...
          /**
           * 将newConnections里的socketChannel注册read事件到selector了
           */
          configureNewConnections()
          // register any new responses for writing
          /**
           * 处理response，如果responseQueue里有需要处理的response
           * 需要继续阅读源码，看看什么时候把response放到了responseQueue里面的
           * 譬如需要发送往socket的response，把对应的socket注册一下write事件
           */
          processNewResponses()
          /**
           * 此处进行真正的nio操作
           * 可读数据放到了completedReceives里
           * 可写socket进行了真正的数据发送
           */
          poll()

          /**
           * 处理接收到的数据，也就是completedReceives里的数据
           */
          processCompletedReceives()
          /**
           * 消息发送完了，最重要的一步操作是 unmute 对应的socket
           */
      ...
      try {
        openOrClosingChannel(receive.source) match {
          case Some(channel) =>
            ...
            // 把request加到了requestChannel中了
            requestChannel.sendRequest(req)

            /**
             * mute操作就是取消对该socket的read事件监听，
             * 保证了该socket传过来的数据处理完了，再从这个socket上read新数据，
             * 保证了处理过程中同一socket上传来的数据有序的处理（写入到log文件中）
             */
